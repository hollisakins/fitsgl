"""Drift guards for the vendored SSG viewer bundle (fitsgl/_viewer).

The viewer is a committed build artifact of `@fitsgl/core` + the `viewer/` app.
...
import fitsgl.fitsgl_config as fc

REPO = Path(__file__).resolve().parents[2]  # tests -> fitsgl-py -> repo root
BUILD_INFO = REPO / "fitsgl-py" / "src" / "fitsgl" / "_viewer" / ".build-info.json"
_SOURCE_ROOTS = (REPO / "fitsgl-core" / "src", REPO / "viewer" / "src")
...
def test_vendored_viewer_is_fresh():
    ...
    assert info["sourceHash"] == _source_hash(_source_rels()), (
        "the vendored viewer bundle is stale relative to @fitsgl/core + viewer source; "
