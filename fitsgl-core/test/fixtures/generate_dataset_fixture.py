...
`dataset.test.ts` validates the EXACT bytes the `fitsgl` writer emits (not a
hand-authored JSON), so a writer/loader schema drift is caught. Three same-grid
bands + a default RGB assignment, written via `fitsgl.dataset.write_dataset`.

Run from anywhere (needs `fitsgl` importable, e.g. PYTHONPATH=../../fitsgl-py/src):
...
from fitsgl.dataset import DatasetBand, DatasetManifest, grid_hash, write_dataset
