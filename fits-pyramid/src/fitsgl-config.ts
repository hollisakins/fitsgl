...
export interface FitsglBandStats {
  ...
  trilogy?: {
    mean: number;
    sigma: number;
    tail: { p99: number; p99_9: number; p99_99: number; p99_999: number; max: number };
    min: number;
  };
}
...
function asTrilogyStats(v: unknown, name: string): FitsglBandStats['trilogy'] | undefined {
  ...
  return {
    ...
    tail: {
      ...
      max: asFiniteNum(t.max, `band "${name}" stats.trilogy.tail.max`),
    },
    min: asFiniteNum(v.min, `band "${name}" stats.trilogy.min`),
  };
}
